...
const HomePage: React.FC<HomePageProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetching data from the tables created in the SQL step
        const [s, g, a, h] = await Promise.all([
          supabase.from('site_stats').select('*').order('display_order'),
          supabase.from('gatherings').select('*').eq('is_next_gathering', true).maybeSingle(),
          supabase.from('announcements').select('*').eq('is_active', true).limit(3),
          supabase.from('monthly_hosts').select('*').eq('is_current_month', true).maybeSingle()
        ]);
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="font-serif italic text-[#7a0000]">Loading Sacred Content...</p>
        </div>
      </div>
    );
  }
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">
                      {news.category} — {new Date(news.publish_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                    </p>
                  </div>
                </div>
